  ...
  constructor(public alertController: AlertController,private routerOutlet: IonRouterOutlet,public modalController: ModalController,private iab: InAppBrowser,private route: ActivatedRoute,public server : ServerService,public toastController: ToastController,private nav: NavController,public loadingController: LoadingController,private stripe: Stripe,private payPal: PayPal){
   ...
   this.customizations = {title: 'New Order', description: 'Food Delivery App Payment', logo: 'https://dicont.s3.amazonaws.com/static/flow-logos/flutterwave-logo.png'}
    ...
    this.route.queryParams.subscribe(params => {
      ...
      this.addressName = (params?.address != undefined || null) ? params?.address : '';
    });
...
async loadData()
{
  const loading = await this.loadingController.create({
    ...
    for (let index = 0; index < this.address.length; index++) {
      if(this.address[index]?.address === this.addressName){
  ...
  async book()
  {
    ...
    console.log("This data value:::" + JSON.stringify(this.data));
...
async getPaymentMethod() {
  ...
  /*makePaymentCallback(response: PaymentSuccessResponse): void {
   ...
   {
    this.presentToast("There is an problem in your payment. Please try after some time.");
   }
