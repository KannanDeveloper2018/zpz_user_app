...
export class SignupPage implements OnInit {
  ...
  async verify(data)
  {
    ...
    this.server.verifyUser(this.user_id).subscribe((response:any) => {
      ...
      this.server.joiningBonus(this.user_id).subscribe(async(res: any) => {
        if(res.bonus != null){
          await this.presentToast("Welcome bonus Rs." + res?.bonus + ", added in your wallet. Go and order food");
        }
      });
